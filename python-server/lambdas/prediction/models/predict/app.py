...
def detect_face(image, transforms=Transforms('facial_markings')):
    ...
    model1.load_state_dict(torch.load(f'detect_models/face_landmarks_small.pth', map_location=torch.device('cpu')))

    model2 = create_detect_model()
    model2.load_state_dict(torch.load('detect_models/face_landmarks.pth', map_location=torch.device('cpu')))
    ...
    with torch.inference_mode():
        ...
        model1_preds = back_to_normal(np.array(model1_preds[0]).reshape(-1, 2), left, top, width, height)
    ...
    with torch.inference_mode():
        ...
        model2_preds = back_to_normal(model2_preds[0].clone().detach().numpy(), left, top, width, height)
